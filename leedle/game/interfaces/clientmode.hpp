...
class ClientModeShared : public Interface<ClientModeShared> {
public:
    ...
    static auto capture_interface() {
        return Interface<ClientModeShared>(
			memory::MemoryScanner("48 8B 0D ? ? ? ? 48 8B 01 FF 50 28 48 8B 05 ? ? ? ?")
			.scan<ClientModeShared>(memory::client)
			.absolute(0x3, 0x7).get_pointer()
		);
    }
};

DEFINE_INTERFACE(ClientModeShared, CLIENTMODE);
