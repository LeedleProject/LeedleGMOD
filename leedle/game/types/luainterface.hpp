...
struct ILuaInterface {
    ...
    virtual const char* get_string(int iStackPos = -1, unsigned int* iOutLen = 0) = 0;
    ...
    virtual void run_string(
        char const* filename,
        char const* path,
        char const* stringtoun,
        bool run = true,
        bool showerrors = true) = 0;

    auto get_state() {
        return (lua_State*)*((uintptr_t*)this + 1);
    }
};
